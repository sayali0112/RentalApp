...
object CloudServices {
    ...
    * */
    ...
    /**
     * This method is use to notify current car speed to admin portal(if FirebaseManager
     * available using firebase cloud function otherwise AwsManager will use)
     * @param speed The current car Speed
     **/
    fun notifyCurrentSpeedToServer(speed: Float) {
        if(notificationChannel == FirebaseManager()){
            FirebaseManager().sendNotification()
        }else{
            AwsManager().sendNotification()
        }
    }
}